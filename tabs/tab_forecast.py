            ...
            try:
                ...
                if result:
                    ...
                    with col_l:
                        ...
                        folium.TileLayer(
                            tiles=result['precip_tile_url'], attr='GEE·GFS',
                    ...
                    with col_r:
                        ...
                        tm = folium.Map(location=map_center, zoom_start=9,
                                        tiles='CartoDB dark_matter')
                        folium.TileLayer(
                            tiles=result['temp_tile_url'], attr='GEE·GFS',
                    ...
                    st.warning('No GFS forecast data available. '
                               'GFS data may have a ~6h delay from the latest run.')
    ...
    if st.button('TRAIN & FORECAST', key='lstm_btn', use_container_width=True):
        with st.spinner('Extracting training data from ERA5 (2018-2023 monsoon seasons)...'):
            try:
                ...
                st.success(f'Model ready ({forecaster.model_type if hasattr(forecaster, "model_type") else "GBM/LSTM"})')
                ...
                with st.spinner('Fetching recent climate data...'):
                    recent = get_era5_timeseries(aoi_json, start_date, end_date,
        ...
        with st.spinner('Running enhanced SAR flood classification...'):
            try:
                ...
                result = classifier.predict_for_aoi(
                    aoi_json,
                    f_start=params['f_start'], f_end=params['f_end'],
                    p_start=params['p_start'], p_end=params['p_end'],
                ...
                if result:
                    ...
                    folium.TileLayer(
                        tiles=result['tile_url'], attr='GEE·Prithvi',
                        name=layer_name, opacity=0.85,
                    ).add_to(m)
                    folium.GeoJson(
                        json.loads(aoi_json),
                        style_function=lambda _: {
                    ...
                    st.warning('Classification returned no results. '
                               'Ensure SAR imagery is available for the selected dates.')
            except Exception as e:
            ...
            with st.spinner('Computing river discharge estimates...'):
                try:
                    ...
                    if result:
                        ...
                        dm = folium.Map(location=map_center, zoom_start=10,
                                        tiles='CartoDB dark_matter')
                        folium.TileLayer(
                            tiles=result['tile_url'], attr='GEE·ERA5·HydroSHEDS',
                            name='Discharge Proxy', opacity=0.85,
                        ).add_to(dm)
                        folium.GeoJson(
                            json.loads(aoi_json),
                            style_function=lambda _: {
            ...
            with st.spinner('Fitting Gumbel distribution to 20-year monsoon maxima...'):
                try:
                    ...
                    if rp:
                        ...
                        rp_df = pd.DataFrame([
                            {'Return Period (yr)': t, 'Discharge Index': v}
                            for t, v in rp['return_levels'].items()
                        ])
            ...
            with st.spinner('Comparing GFS forecast against discharge return levels...'):
                try:
                    ...
                    if exc:
                        ...
                        st.warning('Could not evaluate exceedance. '
                                   'Ensure GFS data and historical baseline are available.')
                except Exception as e:
            ...
            try:
                ...
                if result:
                    ...
                    folium.TileLayer(
                        ...
                        name=result['model_name'], opacity=0.85,
                    ).add_to(m)
                    folium.GeoJson(
                        json.loads(aoi_json),
                        style_function=lambda _: {
