...
def render_projections_tab(aoi_json, params):
    ...
    with st.expander("ERA5-LAND HISTORICAL CLIMATE", expanded=True):
        ...
        era5_year = st.selectbox("Year", list(range(2024, 1999, -1)), index=0, key="era5_year")
            ...
            with st.spinner(f"Computing monsoon profile for {era5_year}..."):
                try:
                    ...
                    if monsoon:
                        ...
                        st.dataframe(
                            df[["month_name", "precip_mm", "runoff_mm", "soil_moisture", "et_mm"]],
                            hide_index=True,
    ...
    with st.expander("CMIP6 FUTURE PROJECTIONS", expanded=False):
        ...
        with pc1:
            cmip_scenario = st.selectbox(
                "SSP Scenario", AVAILABLE_SCENARIOS, format_func=lambda s: s.upper(), key="cmip_ssp"
            )
        with pc2:
        ...
        with sc1:
            cmip_start = st.number_input(
                "Start Year", min_value=2025, max_value=2090, value=2030, step=5, key="cmip_start"
            )
        with sc2:
            cmip_end = st.number_input("End Year", min_value=2030, max_value=2100, value=2050, step=5, key="cmip_end")
            ...
            with st.spinner("Computing precipitation change relative to baseline..."):
                try:
                    ...
                    if change:
                        ...
                        ch2.metric(f"Future ({change['future_period']})", f"{change['future_precip_mm']:.0f} mm/yr")
                        ...
                        cm = folium.Map(location=map_center, zoom_start=9, tiles="CartoDB dark_matter")
                        folium.TileLayer(
                            tiles=change["change_tile_url"],
                            attr="GEE·CMIP6",
                            name="Precip Change %",
                            opacity=0.8,
                        ).add_to(cm)
    ...
    with st.expander("FUTURE FLOOD RISK MAP", expanded=False):
        ...
        with fr1:
            risk_scenario = st.selectbox(
                "SSP Scenario", AVAILABLE_SCENARIOS, format_func=lambda s: s.upper(), key="risk_ssp"
            )
        with fr2:
        ...
        with fr3:
            risk_start = st.number_input(
                "Start Year", min_value=2025, max_value=2090, value=2040, step=5, key="risk_start"
            )
        with fr4:
            risk_end = st.number_input("End Year", min_value=2030, max_value=2100, value=2060, step=5, key="risk_end")
                ...
                try:
                    ...
                    if current and future:
                        ...
                        with map_l:
                            ...
                            cm_now = folium.Map(location=map_center, zoom_start=10, tiles="CartoDB dark_matter")
                            folium.TileLayer(
                                tiles=current["tile_url"],
                                attr="GEE·RF",
                                name="Current Risk",
                                opacity=0.85,
                            ).add_to(cm_now)
                        ...
                        with map_r:
                            ...
                            cm_fut = folium.Map(location=map_center, zoom_start=10, tiles="CartoDB dark_matter")
                            folium.TileLayer(
                                tiles=future["tile_url"],
                                attr="GEE·RF·CMIP6",
                                name="Future Risk",
                                opacity=0.85,
                            ).add_to(cm_fut)
                        ...
                        st.warning(
                            "CMIP6 data not available for this scenario/model/period. "
                            "Current risk map was generated but future projection failed."
                        )
                    else:
                        st.error("Could not generate risk predictions. Ensure the AOI has sufficient data coverage.")
                except Exception as e:
    ...
    with st.expander("MULTI-MODEL ENSEMBLE + RISK CHANGE", expanded=False):
        ...
        e1, e2 = st.columns(2)
        with e1:
            ens_scenario = st.selectbox(
                "SSP Scenario", AVAILABLE_SCENARIOS, format_func=lambda s: s.upper(), key="ens_ssp"
            )
        with e2:
        ...
        e3, e4 = st.columns(2)
        with e3:
            ens_start = st.number_input(
                "Start Year", min_value=2025, max_value=2090, value=2040, step=5, key="ens_start"
            )
        with e4:
            ens_end = st.number_input("End Year", min_value=2030, max_value=2100, value=2060, step=5, key="ens_end")
            ...
            with st.spinner(f"Running {ens_n_models}-model ensemble ({ens_scenario.upper()})..."):
                try:
                    ...
                    result = predictor.predict_future_risk_ensemble(
                        aoi_json,
                        scenario=ens_scenario,
                        models=AVAILABLE_MODELS[:ens_n_models],
                        start_year=ens_start,
                        end_year=ens_end,
                    )
                    ...
                    if result:
                        ...
                        with col_a:
                            ...
                            m1 = folium.Map(location=map_center, zoom_start=10, tiles="CartoDB dark_matter")
                            folium.TileLayer(
                                tiles=result["current_tile_url"],
                                attr="GEE·RF",
                                name="Current Risk",
                                opacity=0.85,
                            ).add_to(m1)
                        ...
                        with col_b:
                            ...
                            m2 = folium.Map(location=map_center, zoom_start=10, tiles="CartoDB dark_matter")
                            folium.TileLayer(
                                tiles=result["ensemble_tile_url"],
                                attr="GEE·RF·ENS",
                                name="Ensemble Risk",
                                opacity=0.85,
                            ).add_to(m2)
                        ...
                        with col_c:
                            ...
                            m3 = folium.Map(location=map_center, zoom_start=10, tiles="CartoDB dark_matter")
                            folium.TileLayer(
                                tiles=result["delta_tile_url"],
                                attr="GEE·DELTA",
                                name="Risk Change",
                                opacity=0.85,
                            ).add_to(m3)
                        ...
                        with st.expander("PER-MODEL BREAKDOWN", expanded=False):
                            ...
                            for gcm, dist in pm_dist.items():
                                ...
                                pm_rows.append(
                                    {
                                        "Model": gcm,
                                        **{f"Class {c}%": round(dist.get(c, 0) / total * 100, 1) for c in range(1, 6)},
                                    }
                                )
                        ...
                        st.warning(
                            "Ensemble projection failed. Check CMIP6 data availability "
