...
def render_mca_tab(aoi_json, params):
    ...
    with c1:
        st.markdown(
            """<div class="metric-card"><div class="metric-label">MODEL STATUS</div>
            <div class="metric-value">MCA</div><div class="metric-sub">Multi-Criteria Analysis · Active</div></div>""",
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            """<div class="metric-card"><div class="metric-label">RESOLUTION</div>
            <div class="metric-value">30m</div><div class="metric-sub">Native GEE pixel scale</div></div>""",
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    with c2:
        ...
        map_data = st_folium(
            m1, height=560, use_container_width=True, key="tab1_map", returned_objects=["last_clicked"]
        )
    ...
    with st.expander("URBAN FLOOD VULNERABILITY INDEX  [UFVI]", expanded=False):
        st.markdown(
            '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin-bottom:8px;">IMPERVIOUSNESS · ELEVATION · SLOPE · POPULATION · DRAINAGE PROXY</div>',
            unsafe_allow_html=True,
        )
        if st.button("COMPUTE UFVI", key="ufvi_btn", use_container_width=True):
            with st.spinner("Computing Urban Flood Vulnerability Index..."):
                try:
                    ...
                    if ufvi:
                        ...
                        fol.TileLayer(tiles=ufvi["tile_url"], attr="GEE·UFVI", name="UFVI", opacity=0.8).add_to(
                            ufvi_map
                        )
                        fol.GeoJson(
                            json.loads(aoi_json),
            ...
            st.info(
                "Click to compute the Urban Flood Vulnerability Index combining imperviousness, elevation, slope, and population."
            )
