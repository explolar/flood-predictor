...
def render_progression_tab(aoi_json, params):
    ...
    st.markdown(
        f"<div style=\"font-family:'Inter',sans-serif;font-size:0.78rem;letter-spacing:2px;color:rgba(144,202,249,0.4);margin-bottom:8px;\">MONSOON FLOOD PROGRESSION · {prog_year}</div>",
    ...
    if prog_df is not None and not prog_df.empty:
        ...
        with t4c1:
            ...
            st.markdown(
                f'<div class="stats-row"><div class="stat-chip"><b>{total_rain:.0f} mm</b>Season Total</div><div class="stat-chip"><b>{peak_month}</b>Peak Month</div></div>',
                unsafe_allow_html=True,
            )
        with t4c2:
            ...
            with st.spinner(f"Computing SAR flood mask · {prog_month} {prog_year}..."):
                prog_tile = get_month_sar_tile(
                    aoi_json, prog_year, prog_month_num, polarization, f_threshold, apply_speckle
                )
            ...
            if prog_tile:
                folium.TileLayer(
                    tiles=prog_tile, attr="GEE", name=f"Flood · {prog_month} {prog_year}", opacity=0.85
                ).add_to(m4)
        ...
        chips_html = "".join(
            f'<span class="prog-month-chip">{row["Month"]} · {row["Rain (mm)"]} mm</span>'
        ...
        with st.expander("FLOOD TIMELAPSE ANIMATION", expanded=False):
            st.markdown(
                '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin-bottom:8px;">LEAFLET.JS ANIMATION · MONTHLY SAR FRAMES · PLAY/PAUSE</div>',
                unsafe_allow_html=True,
            )
                ...
                with st.spinner("Generating monthly SAR flood tiles for animation..."):
                    try:
                        ...
                        for m_num, m_name in month_map.items():
                            tile = get_month_sar_tile(
                                aoi_json, prog_year, m_num, polarization, f_threshold, apply_speckle
                            )
                            if tile:
