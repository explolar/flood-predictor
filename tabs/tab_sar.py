...
from gee_functions.layers import get_jrc_flood_history, get_ndvi_tile
...
def render_sar_tab(aoi_json, params):
    ...
    sar_sub1, sar_sub2, sar_sub3 = st.tabs(["  DETECTION  ", "  COMPARISON  ", "  PROGRESSION  "])
    with sar_sub1:
...
def _render_detection(aoi_json, params):
    ...
    try:
        with st.spinner(f"Querying Sentinel-1 {polarization} SAR archive..."):
            sar = get_all_sar_data(
                aoi_json, str(f_start), str(f_end), str(p_start), str(p_end), f_threshold, polarization, apply_speckle
            )
    except Exception as e:
        sar = None
        st.error(f"SAR computation failed: {e}")
    if sar is None:
        st.error("No Sentinel-1 data found for the selected dates/AOI. Try adjusting the date windows or AOI.")
        return
    ...
    with col1:
        st.markdown(
            f"""<div class="metric-card"><div class="metric-label">INUNDATED AREA</div>
            <div class="metric-value">{sar["area_ha"]}</div><div class="metric-sub">Hectares · post-event SAR</div></div>""",
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            f"""<div class="metric-card"><div class="metric-label">POPULATION EXPOSED</div>
            <div class="metric-value" style="font-size:1.8rem;">{sar["pop_exposed"]:,}</div><div class="metric-sub">WorldPop 2020 · 100m</div></div>""",
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            f"""<div class="metric-card"><div class="metric-label">THRESHOLD · POLAR.</div>
            <div class="metric-value" style="font-size:1.6rem;">{f_threshold} dB</div><div class="metric-sub">{polarization} · {"Speckle filtered" if apply_speckle else "Raw"}</div></div>""",
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
        sar_view = st.radio(
    ...
    with col2:
        ...
        MiniMap(
            tile_layer="CartoDB dark_matter", position="bottomright", toggle_display=True, zoom_level_offset=-6
        ).add_to(m2)
        ...
        if sar_view == "Flood Depth" and depth_data:
            ...
            if depth_data.get("histogram"):
                ...
                st.markdown(
                    '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin:10px 0 4px;">FLOOD DEPTH DISTRIBUTION · PIXEL COUNT PER DEPTH BAND</div>',
                    unsafe_allow_html=True,
                )
                hist_df = pd.DataFrame({"Depth Band (m)": list(hist.keys()), "Pixels": list(hist.values())}).set_index(
                    "Depth Band (m)"
                )
    ...
    with st.expander("CHIRPS RAINFALL TIME SERIES", expanded=False):
        ...
        if rain_df is not None and not rain_df.empty:
            st.markdown(
                '<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;color:rgba(144,202,249,0.5);letter-spacing:2px;margin-bottom:8px;">MEAN DAILY RAINFALL (mm) · AOI AVERAGE</div>',
                unsafe_allow_html=True,
            )
    ...
    with st.expander("FLOOD RETURN PERIOD ANALYSIS  [Gumbel Distribution]", expanded=False):
        ...
        if rp_data:
            ...
            st.markdown(
                '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin:12px 0 6px;">RETURN PERIOD TABLE · MONSOON TOTAL (Jun–Oct)</div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                f'<table class="rp-table"><tr><th>RETURN PERIOD</th><th>RAINFALL</th><th>RELATIVE</th></tr>{rows_html}</table>',
                unsafe_allow_html=True,
            )
            st.markdown(
                f'<div style="font-size:0.7rem;color:#3a5060;font-family:JetBrains Mono,monospace;margin-top:10px;">Based on {rp_data["n_years"]} years CHIRPS · Gumbel extreme value distribution</div>',
                unsafe_allow_html=True,
            )
        else:
    ...
    with st.expander("CROP LOSS ASSESSMENT  [NDVI × WorldCover Cropland]", expanded=False):
        ...
        if crop_data:
            ...
            st.markdown(
                '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin:10px 0 4px;">NDVI DAMAGE ON CROPLAND · GREEN = HEALTHY · RED = DAMAGED</div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                f'<div style="font-size:0.7rem;color:#3a5060;font-family:JetBrains Mono,monospace;margin-top:8px;">Crop: {crop_type} · Price: ₹{crop_price:,}/ha · NDVI drop threshold: 0.10 · Source: Sentinel-2 SR + ESA WorldCover</div>',
                unsafe_allow_html=True,
            )
        else:
    ...
    with st.expander("JRC HISTORICAL FLOOD FREQUENCY  [1984 – 2021]", expanded=False):
        ...
        if jrc_hist:
            jrc_df = pd.DataFrame({"Year": list(jrc_hist.keys()), "Flood Months": list(jrc_hist.values())}).set_index(
                "Year"
            )
    ...
    with st.expander("ROAD NETWORK IMPACT  [OSM · Evacuation Routes]", expanded=False):
        ...
        if road_data and road_data["roads"]:
            ...
            km_df = (
                pd.DataFrame(list(road_data["km_by_type"].items()), columns=["Highway Type", "Length (km)"])
                .sort_values("Length (km)", ascending=False)
                .set_index("Highway Type")
            )
        ...
        if dam_list:
            ...
            for dam in dam_list:
                ...
                folium.CircleMarker(
                    ...
                    fill_color="#00BFFF",
                    fill_opacity=0.6,
                    tooltip=f"🏞 {dam['name']} · {dam['river']}<br>Capacity: {int(cap):,} MCM · Use: {dam['main_use']} · Built: {dam['year']}",
                ).add_to(dam_map)
                ...
                [
                    {
                        "Name": d["name"],
                        "River": d["river"],
                        "Capacity (MCM)": int(d["capacity_mcm"]),
                        "Use": d["main_use"],
                        "Built": d["year"],
                    }
                    for d in dam_list
    ...
    with st.expander("FLOOD RECESSION TRACKER  [SAR · +12 / +24 / +36 days]", expanded=False):
        with st.spinner("Computing post-event SAR flood extent (4 × 12-day windows)..."):
            recession = get_recession_data(
                aoi_json, str(f_end), str(p_start), str(p_end), polarization, f_threshold, apply_speckle
            )
        if recession:
        ...
        if st.button("ASSESS BUILDING DAMAGE", key="bldg_dmg_btn", use_container_width=True):
            with st.spinner("Cross-referencing buildings with flood depth..."):
                try:
                    ...
                    bldg = get_building_damage(
                        aoi_json, f_start, f_end, p_start, p_end, f_threshold, polarization, apply_speckle
                    )
                    if bldg:
                        ...
                        folium.TileLayer(tiles=bldg["tile_url"], attr="GEE·GOB", name="Building Damage").add_to(
                            bldg_map
                        )
                        folium.GeoJson(
                            json.loads(aoi_json),
