...
from gee_functions.indices import (
    INDEX_REGISTRY,
    diagnose_s2_access,
    get_all_index_tiles,
    get_index_download_url,
    get_index_thumb_url,
)
...
def render_indices_tab(aoi_json, params):
    ...
    if compute_clicked:
        ...
        if all_results:
            ...
            st.success(
                f"Computed {len(all_results)} / 7 indices from composite ({all_results[list(all_results.keys())[0]]['n_scenes']} scenes)"
            )
    ...
    for tab_widget, index_key in zip(sub_tabs, index_keys):
        with tab_widget:
            _render_single_index(index_key, aoi_json, aoi, map_center, date_start, date_end, cloud_thresh)


def _render_single_index(index_key, aoi_json, aoi, map_center, date_start, date_end, cloud_thresh):
    ...
    with col_ctrl:
        ...
        download_url = get_index_download_url(aoi_json, index_key, date_start, date_end, cloud_thresh)
        if download_url:
            st.link_button(
        ...
        try:
            thumb_url = get_index_thumb_url(aoi_json, index_key, date_start, date_end, cloud_thresh)
    ...
    with col_map:
        ...
        m.get_root().html.add_child(folium.Element(get_index_legend(m.get_name(), index_key)))
...
def _render_info_expander(index_key, meta):
    ...
    with st.expander(f"INFO & METHODOLOGY — {index_key}", expanded=False):
        st.markdown(
            ...
            'color:rgba(144,202,249,0.4);letter-spacing:2px;margin-bottom:10px;">'
            "CLASSIFICATION BASIS · SENTINEL-2 SR · COPERNICUS</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
