...
def render_multiyear_tab(aoi_json, params):
    ...
    st.markdown(
        "<div style=\"font-family:'Inter',sans-serif;font-size:0.78rem;letter-spacing:2px;color:rgba(144,202,249,0.4);margin-bottom:8px;\">MULTI-YEAR MONSOON FLOOD COMPARISON · SAR-BASED</div>",
        unsafe_allow_html=True,
    )

    years_available = list(range(2017, 2025))
    selected_years = st.multiselect(
        "Select years to compare", years_available, default=[2020, 2021, 2022, 2023, 2024], key="multiyear_select"
    )
        ...
        if len(selected_years) < 2:
            ...
            if result and result["data"] is not None:
                ...
                if len(years_with_tiles) >= 2:
                    st.markdown(
                        '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin:14px 0 6px;">FLOOD EXTENT MAPS</div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    with col_a:
                        ...
                        st.markdown(
                            f'<div style="text-align:center;color:#00FFFF;font-size:0.8rem;letter-spacing:2px;">{yr}</div>',
                            unsafe_allow_html=True,
                        )
