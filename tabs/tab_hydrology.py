...
def _render_streams(aoi_json, map_center):
    ...
    with col_map:
        ...
        if stream_layer == "Stream Order":
            folium.TileLayer(
                tiles=hydro["stream_url"],
                attr="GEE · HydroSHEDS",
                name="Stream Order",
                opacity=0.9,
            ).add_to(m)
            m.get_root().html.add_child(folium.Element(get_stream_order_legend(m.get_name())))
        else:
            folium.TileLayer(
                tiles=hydro["flow_acc_url"],
                attr="GEE · HydroSHEDS",
                name="Flow Accumulation (log)",
                opacity=0.8,
            ).add_to(m)
            m.get_root().html.add_child(folium.Element(get_flow_acc_legend(m.get_name())))
...
def _render_hand(aoi_json, map_center):
    ...
    with ctrl_c2:
        flood_depth = st.slider(
            ...
            value=0.0,
            step=0.5,
            help="Set > 0 to simulate flood inundation. Pixels with HAND <= this depth are considered flooded.",
    ...
    if st.button("COMPUTE HAND", key="hand_btn", use_container_width=True):
        with st.spinner("Computing Height Above Nearest Drainage..."):
            try:
                hand = get_hand_data(aoi_json, stream_threshold=hand_threshold, flood_depth_m=flood_depth_val)
            except Exception as e:
        ...
        with col_map:
            ...
            if hand_layer == "HAND Surface":
                ...
                m.get_root().html.add_child(folium.Element(get_hand_legend(m.get_name())))
            elif hand_layer == "Flood Extent":
                folium.TileLayer(
                    tiles=hand["flood_url"],
                    attr="GEE · HAND",
                    name=f"Flood Extent ({flood_depth_val}m)",
                    opacity=0.8,
                ).add_to(m)
            elif hand_layer == "Flood Depth":
                folium.TileLayer(
                    tiles=hand["depth_url"],
                    attr="GEE · HAND",
                    name=f"Flood Depth (0-{flood_depth_val}m)",
                    opacity=0.85,
                ).add_to(m)
