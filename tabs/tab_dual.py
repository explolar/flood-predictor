...
def render_dual_tab(aoi_json, params):
    ...
    st.markdown(
        "<div style=\"font-family:'Inter',sans-serif;font-size:0.78rem;letter-spacing:2px;color:rgba(144,202,249,0.4);margin-bottom:8px;\">SYNCHRONIZED PRE / POST SAR BACKSCATTER COMPARISON</div>",
    ...
    if sar_d is None:
        st.error(
            "SAR analysis failed — no Sentinel-1 data found for the selected dates/AOI. Try adjusting dates or AOI."
        )
        return
    st.markdown(
        '<div class="dual-label-row"><div class="dual-label">◀ PRE-FLOOD SAR</div><div class="dual-label">POST-FLOOD SAR ▶</div></div>',
        unsafe_allow_html=True,
    )
    dmap = DualMap(location=map_center, zoom_start=11, tiles=None, layout="horizontal")
    folium.TileLayer(
        tiles="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", attr="CartoDB", name="Basemap"
    ).add_to(dmap.m1)
    folium.TileLayer(
        tiles="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", attr="CartoDB", name="Basemap"
    ).add_to(dmap.m2)
    ...
    st.markdown(
        "<div style=\"font-family:'Inter',sans-serif;font-size:0.78rem;letter-spacing:2px;color:rgba(144,202,249,0.4);margin-bottom:8px;\">SENTINEL-2 TRUE COLOR · PRE vs POST</div>",
        unsafe_allow_html=True,
    )
    ...
    if s2_rgb:
        st.markdown(
            '<div class="dual-label-row"><div class="dual-label">◀ PRE-FLOOD TRUE COLOR</div><div class="dual-label">POST-FLOOD TRUE COLOR ▶</div></div>',
            unsafe_allow_html=True,
        )
    ...
    with st.expander("3D TERRAIN FLOOD VIEW  [PyDeck]", expanded=False):
        st.markdown(
            '<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:rgba(144,202,249,0.4);letter-spacing:2px;margin-bottom:8px;">SRTM DEM · PYDECK COLUMNLAYER · INTERACTIVE 3D</div>',
            unsafe_allow_html=True,
        )
