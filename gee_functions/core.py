...
def get_aoi_stats(aoi_json):
    ...
    area_m2 = aoi_geom.area(maxError=1).getInfo() or 0
    return {
        "elev_min": round(stats.get("elev_min", 0) or 0),
        "elev_max": round(stats.get("elev_max", 0) or 0),
        "elev_mean": round(stats.get("elev_mean", 0) or 0),
        "slope_mean": round((stats.get("slope_mean", 0) or 0), 1),
