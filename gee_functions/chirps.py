...
def get_chirps_series(aoi_json, start_str, end_str):
    ...
    if not fc or not fc.get("features"):
        return None
    records = [
        {"date": f["properties"]["date"], "rainfall_mm": float(f["properties"]["rain"])}
...
def get_return_period(aoi_json):
    try:
        ...
        rains = [float(f["properties"]["rain"]) for f in data["features"] if f["properties"].get("rain") is not None]
        if len(rains) < 10:
            return None
        n = len(rains)
        mu = sum(rains) / n
...
def get_progression_stats(aoi_json, year):
    try:
        ...
        records = [
            {
                "Month": names[int(f["properties"]["month"])],
