...
def _get_cmip6_collection(aoi_geom, scenario, model, start_year, end_year):
    ...
    return (ee.ImageCollection(_CMIP6_ASSET)
...
def get_cmip6_projections(aoi_json, scenario='ssp245', model='ACCESS-CM2',
    ...
    mean_precip = (col.select('pr').mean()
                   .multiply(_PR_TO_MM_DAY * 365)
                   .clip(aoi_geom).rename('precip'))
    ...
    stats = ee.Image.cat([mean_precip, mean_tasmax, mean_tasmin]).reduceRegion(
...
@st.cache_data(show_spinner=False, ttl=7200)
def get_cmip6_scenario_comparison(aoi_json, model='ACCESS-CM2',
                                   periods=None):
    """
...
def get_cmip6_risk_feature_stack(aoi_json, scenario='ssp245', model='ACCESS-CM2',
                                 start_year=2030, end_year=2050):
    """
    ...
    future_precip = (col.select('pr').mean()
                     .multiply(_PR_TO_MM_DAY * 365)
...
@st.cache_data(show_spinner=False, ttl=7200)
def get_cmip6_precip_change(aoi_json, scenario='ssp245', model='ACCESS-CM2',
                             future_start=2040, future_end=2060):
    """
    ...
    pct_change = (future_precip.subtract(baseline_precip)
                  .divide(baseline_precip.max(1))
                  .multiply(100).rename('pct_change'))

    stats_baseline = baseline_precip.rename('val').reduceRegion(
        reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=25000, maxPixels=1e8
    ).getInfo() or {}
    stats_future = future_precip.rename('val').reduceRegion(
        reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=25000, maxPixels=1e8
    ).getInfo() or {}
    stats_pct = pct_change.reduceRegion(
        reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=25000, maxPixels=1e8
    ).getInfo() or {}
