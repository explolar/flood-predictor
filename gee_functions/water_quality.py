...
def get_turbidity_map(aoi_json, start_date, end_date):
    ...
    ndti_stats = (
        ndti.reduceRegion(
            reducer=ee.Reducer.mean().combine(ee.Reducer.minMax(), "", True), geometry=aoi_geom, scale=20, maxPixels=1e9
        ).getInfo()
        or {}
    )

    chl_stats = (
        chl_a.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=20, maxPixels=1e9).getInfo() or {}
    )

    # Tile URLs
    ndti_tile = ndti.getMapId(
        {"min": -0.3, "max": 0.3, "palette": ["2166ac", "67a9cf", "d1e5f0", "fddbc7", "ef8a62", "b2182b"]}
    )["tile_fetcher"].url_format

    chl_tile = chl_a.getMapId({"min": 0.5, "max": 2.0, "palette": ["f7fcf5", "c7e9c0", "74c476", "238b45", "00441b"]})[
        "tile_fetcher"
    ].url_format
