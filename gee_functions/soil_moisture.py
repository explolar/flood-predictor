...
def get_soil_moisture_data(aoi_json, start_date, end_date):
    ...
    # Time-series
    def extract_sm(img):
        stats = img.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=9000, maxPixels=1e8)
        return ee.Feature(
            None, {"date": img.date().format("YYYY-MM-dd"), "soil_moisture": stats.get("soil_moisture_am")}
        )
    ...
    if data and data.get("features"):
        for feat in data["features"]:
            ...
            if props.get("soil_moisture") is not None:
                records.append({"date": props["date"], "soil_moisture": round(float(props["soil_moisture"]), 4)})
    ...
    tile_url = latest.getMapId({"min": 0.0, "max": 0.5, "palette": ["f7fcb1", "addd8e", "41ab5d", "006837", "004529"]})[
        "tile_fetcher"
    ].url_format

    # Mean soil moisture
    mean_stats = (
        latest.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=9000, maxPixels=1e8).getInfo() or {}
    )
...
def get_smap_tile(aoi_json, target_date=None):
    ...
    try:
        ...
        return img.getMapId({"min": 0.0, "max": 0.5, "palette": ["f7fcb1", "addd8e", "41ab5d", "006837", "004529"]})[
            "tile_fetcher"
        ].url_format
    except Exception:
        return None
