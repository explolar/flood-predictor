...
def get_era5_climate_summary(aoi_json, year=2023):
    ...
    stats = (
        ...
        .reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=11000, maxPixels=1e8)
        .getInfo()
        or {}
    )
    ...
    tile_url = (
        total_runoff.rename("runoff")
        .getMapId({"min": 0, "max": 800, "palette": ["ffffcc", "a1dab4", "41b6c4", "2c7fb8", "253494"]})["tile_fetcher"]
        .url_format
    )
...
def get_era5_timeseries(aoi_json, start_date, end_date, variable="total_precipitation_sum"):
    ...
    def extract(img):
        val = img.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=11000, maxPixels=1e8)
        return ee.Feature(
            None,
            {
...
def get_era5_monsoon_profile(aoi_json, year=2023):
    ...
    for month in range(6, 11):
        ...
        stats = (
            ee.Image.cat([precip.rename("precip"), runoff.rename("runoff"), sm.rename("sm"), et.rename("et")])
            .reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=11000, maxPixels=1e8)
            .getInfo()
            or {}
        )
...
def get_era5_features_for_ml(aoi_json, year=2023):
    ...
    """
    aoi_geom = ee.Geometry(json.loads(aoi_json))
    col = ee.ImageCollection(_ERA5_ASSET).filterBounds(aoi_geom).filterDate(f"{year}-01-01", f"{year + 1}-01-01")
    ...
    return precip.addBands(runoff).addBands(sm_shallow).addBands(sm_deep).addBands(temp).clip(aoi_geom)
