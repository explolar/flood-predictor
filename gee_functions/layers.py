...
def get_ndvi_tile(aoi_json, p_start, p_end, f_start, f_end):
    try:
        ...
        col_pre = (
            ...
            .filterDate(str(p_start), str(p_end))
        ...
        return ndvi_diff.getMapId({"min": -0.3, "max": 0.5, "palette": ["1a9850", "ffffbf", "d73027"]})[
            "tile_fetcher"
        ].url_format
    except Exception:
        return None
        ...
        def year_flood_months(y):
            ...
            has_water = monthly.map(
                lambda img: ee.Feature(
                    ...
                    {
                        "w": img.eq(2)
                        .reduceRegion(reducer=ee.Reducer.max(), geometry=aoi_geom, scale=300, maxPixels=1e8)
                        .get("waterClass")
                    },
                )
            )
