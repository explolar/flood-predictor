...
def get_multiyear_flood_comparison(aoi_json, years=None, polarization="VH", threshold=3.0, speckle=True):
    ...
    for year in years:
        try:
            ...
            counts = ee.Dictionary({"pre": pre_col.size(), "post": post_col.size()}).getInfo() or {}
            ...
            if pre_count == 0 or post_count == 0:
                results.append(
                    {"year": year, "flood_area_ha": None, "pre_scenes": pre_count, "post_scenes": post_count}
                )
                continue
            ...
            tile_url = flood.getMapId({"palette": ["FF6B6B"]})["tile_fetcher"].url_format
    ...
    chart = [{"label": str(r["year"]), "value": r["flood_area_ha"] or 0} for r in results]
    tiles = [{"tile_url": url, "metadata": {"year": year}} for year, url in tile_urls.items()]
