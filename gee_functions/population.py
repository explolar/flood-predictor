...
def get_displacement_estimate(aoi_json, flood_mask_threshold=3.0):
    ...
    total_pop_stats = (
        pop.reduceRegion(reducer=ee.Reducer.sum(), geometry=aoi_geom, scale=100, maxPixels=1e9).getInfo() or {}
    )
    ...
    if child_bands:
        ...
        child_stats = (
            children.reduceRegion(reducer=ee.Reducer.sum(), geometry=aoi_geom, scale=100, maxPixels=1e9).getInfo() or {}
        )
    ...
    if elderly_bands:
        ...
        elderly_stats = (
            elderly.reduceRegion(reducer=ee.Reducer.sum(), geometry=aoi_geom, scale=100, maxPixels=1e9).getInfo() or {}
        )
