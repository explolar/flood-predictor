...
def get_spi_index(aoi_json, target_year=2024, baseline_years=20):
    ...
    # Mean SPI for AOI
    spi_stats = (
        spi.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=5000, maxPixels=1e8).getInfo() or {}
    )
    ...
    tile_url = spi.getMapId(
        {
            "min": -3,
            "max": 3,
            "palette": ["67001f", "d6604d", "f4a582", "fddbc7", "f7f7f7", "d1e5f0", "92c5de", "4393c3", "2166ac"],
        }
...
def get_ndvi_anomaly(aoi_json, target_year=2024, baseline_years=20):
    ...
    anom_stats = (
        anomaly_scaled.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi_geom, scale=1000, maxPixels=1e8).getInfo()
        or {}
    )
    ...
    tile_url = anomaly_scaled.getMapId(
        {"min": -0.2, "max": 0.2, "palette": ["8c510a", "d8b365", "f6e8c3", "f5f5f5", "c7eae5", "5ab4ac", "01665e"]}
    )["tile_fetcher"].url_format
