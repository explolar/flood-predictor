...
def get_building_damage(aoi_json, f_start, f_end, p_start, p_end, threshold, polarization, speckle):
    ...
    buildings = ee.FeatureCollection("GOOGLE/Research/open-buildings/v3/polygons").filterBounds(aoi_geom)
    ...
    get_all_sar_data(aoi_json, str(f_start), str(f_end), str(p_start), str(p_end), threshold, polarization, speckle)
    ...
    # Rasterize buildings
    buildings_img = (
        buildings.reduceToImage(properties=["area_in_meters"], reducer=ee.Reducer.first())
        .clip(aoi_geom)
        .gt(0)
        .selfMask()
    )
    ...
    # Classify damage
    building_depth.lte(0.5).selfMask()
    building_depth.gt(0.5).And(building_depth.lte(1.5)).selfMask()
    building_depth.gt(1.5).And(building_depth.lte(3.0)).selfMask()
    building_depth.gt(3.0).selfMask()
    ...
    tile_url = damage_img.getMapId({"min": 1, "max": 4, "palette": ["#fee08b", "#fc8d59", "#d73027", "#67001f"]})[
        "tile_fetcher"
    ].url_format

    # Aggregate stats
    stats = (
        damage_img.reduceRegion(
            reducer=ee.Reducer.frequencyHistogram(), geometry=aoi_geom, scale=10, maxPixels=1e9
        ).getInfo()
        or {}
    )
