...
def calculate_flood_risk(aoi_geom, w_lulc=0.40, w_slope=0.30, w_rain=0.30):
    ...
    return (
        lulc_r.multiply(w_lulc / 100)
        .add(slope_r.multiply(w_slope / 100))
        .add(rain_r.multiply(w_rain / 100))
        .clip(aoi_geom)
        .round()
    )
...
def get_mca_tile(aoi_json, w_lulc, w_slope, w_rain):
    ...
    return risk.getMapId({"min": 1, "max": 5, "palette": ["1a9850", "91cf60", "ffffbf", "fc8d59", "d73027"]})[
        "tile_fetcher"
    ].url_format
