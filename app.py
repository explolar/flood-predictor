...
with st.sidebar:
    ...
    st.markdown(
        f'<div style="text-align:center;font-family:JetBrains Mono,monospace;font-size:0.78rem;color:#64B5F6;margin-top:-8px;">{f_threshold} dB · {polarization}</div>',
        unsafe_allow_html=True,
    )
    ...
    crop_price = st.number_input(
        "Price (₹/ha)", min_value=0, value=default_price, step=5000, label_visibility="visible"
    )
    ...
    st.markdown(
        f'<div style="text-align:center;font-family:JetBrains Mono,monospace;font-size:0.72rem;color:#64B5F6;margin-top:-8px;">Scenes ≤ {cloud_thresh}% cloud</div>',
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.aoi:
        report = generate_report(
            ...
            {"area_ha": st.session_state.get("area_ha", 0), "pop_exposed": st.session_state.get("pop_exposed", "N/A")},
        )
        st.download_button(
            "EXPORT TECH REPORT (.txt)", report, file_name="HydroRisk_Atlas_Report.txt", use_container_width=True
        )
