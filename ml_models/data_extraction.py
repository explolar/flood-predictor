...
def extract_risk_training_samples(aoi_json, n_points=5000, scale=100, include_era5=False):
    ...
    feature_names = ["elevation", "slope", "annual_rainfall", "lulc_class", "jrc_occurrence", "jrc_max_extent"]
    ...
    samples = feature_stack.stratifiedSample(
        numPoints=n_points // 5, classBand="risk_class", region=aoi_geom, scale=scale, seed=42, geometries=True
    )
...
@st.cache_data(show_spinner=False, ttl=3600)
def extract_sar_training_samples(
    ...
    samples = feature_stack.stratifiedSample(
        numPoints=n_points // 2, classBand="flood_label", region=aoi_geom, scale=scale, seed=42, geometries=True
    )

    sample_info = samples.getInfo()
    feature_names = ["pre_sar", "post_sar", "sar_diff", "sar_ratio", "elevation", "slope", "jrc_occ", "jrc_season"]
    return _features_from_info(sample_info, feature_names, label_name="flood_label")


def dataframe_to_ee_fc(df, value_col, lat_col="latitude", lon_col="longitude", max_features=4500):
