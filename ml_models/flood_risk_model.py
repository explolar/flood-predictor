...
class FloodRiskPredictor:
    ...
    def predict_future_risk(self, aoi_json, scenario='ssp245', model='ACCESS-CM2',
        ...
        future_stack = get_cmip6_risk_feature_stack(
            aoi_json, scenario=scenario, model=model,
        ...
        samples = future_stack.sample(
            region=aoi_geom, scale=100, numPixels=5000,
    ...
    def predict_future_risk_ensemble(self, aoi_json, scenario='ssp245',
                                     models=None, start_year=2030, end_year=2050):
        """
        ...
        """
        from gee_functions.cmip6 import get_cmip6_risk_feature_stack, AVAILABLE_MODELS
        from ml_models.data_extraction import _features_from_info, dataframe_to_ee_fc
        ...
        for m in models:
            ref_stack = get_cmip6_risk_feature_stack(
                aoi_json, scenario=scenario, model=m,
                start_year=start_year, end_year=end_year,
            )
        ...
        for gcm in models:
            stack = get_cmip6_risk_feature_stack(
                aoi_json, scenario=scenario, model=gcm,
                start_year=start_year, end_year=end_year,
            )
            if stack is None:
                continue
        ...
        ens_image = (fc_ens.reduceToImage(['risk_pred'], ee.Reducer.first())
                     .clip(aoi_geom).rename('risk'))
        ...
        delta_image = (fc_delta.reduceToImage(['risk_delta'], ee.Reducer.first())
                       .clip(aoi_geom).rename('delta'))
        delta_tile = delta_image.getMapId({
            'min': -3, 'max': 3,
            'palette': ['1a9850', '91cf60', 'd9ef8b', 'f7f7f7', 'fee08b', 'fc8d59', 'd73027']
        })['tile_fetcher'].url_format

        # Current risk map tile (for side-by-side)
        fc_cur = dataframe_to_ee_fc(ref_df, 'current_risk')
        cur_image = (fc_cur.reduceToImage(['current_risk'], ee.Reducer.first())
                     .clip(aoi_geom).rename('risk'))
