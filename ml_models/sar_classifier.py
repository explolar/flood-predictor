...
class SARFloodClassifier:
    ...
    def train(self, df):
        ...
        self.model = GradientBoostingClassifier(
            n_estimators=200, max_depth=6, learning_rate=0.1, subsample=0.8, min_samples_leaf=20, random_state=42
        )
        self.model.fit(X, y)
    ...
    def classify_for_aoi(
        ...
        try:
            if return_probability:
                flood_binary = flood_image.gte(0.5)
            else:
                flood_binary = flood_image.mask()
            area_m2 = (
        ...
        threshold_area_ha = None  # threshold area not meaningful for pixel-based calc
